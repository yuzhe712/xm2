...
项目已完成 P0-P4 和 P5 的本地代码、自动化测试、Compose、备份恢复及压测验证。真实 DeepSeek 冒烟仍需使用部署者已轮换的运行时密钥；在该项通过前不会把 P5 标记为全部完成。当前结果只说明本仓库在已记录环境和负载下的表现，不代表生产容量或团队规模承诺。
...
压测窗口暂停 Worker 后，436 个 AI/通知任务形成 870 条队列积压，用于验证任务不会因 Worker 停止而丢失。10 用户超限测试还发现 Nginx 默认把限流返回成 503；配置已修正为 429，随后 200 个并发突发请求得到 42 个 200 和 158 个 429。当前主要性能瓶颈是队列查询，数据增长后 P95 明显高于创建和详情接口。
