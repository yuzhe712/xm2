...
- [ ] 增加前端路由和角色布局。
- [ ] 将 `App.tsx` 拆分为员工、运维、管理员页面。
- [ ] 员工端实现新建、我的工单、详情、评论、确认和重新打开。
- [ ] 运维端实现队列、我的工作、认领、处理时间线和解决表单。
- [ ] 管理端实现用户、团队、SLA 和服务目录最小配置。
- [ ] AI 建议和证据改为工单详情中的辅助面板。
- [ ] 移除空导航、演示账号提示、固定 Mock 模式和伪报表。
- [ ] Web 生产构建作为主产物，Electron 保持可选。
...
- [ ] 编写完整集成测试：员工提交 -> AI 分诊 -> 运维认领 -> 回复 -> 解决 -> 员工关闭。
- [ ] 编写故障测试：LLM 超时、Redis 中断、Worker 重启、重复认领、权限越权。
- [ ] 使用 k6 或 Locust 压测创建、队列查询、详情查询和并发认领。
- [ ] 记录测试机器配置、数据量、并发数、P50/P95/P99、错误率和瓶颈。
- [ ] 更新 README、部署文档、架构图、API 示例和面试问答。
- [ ] 删除“企业级”“支持 N 人”等未经验证的表述。
...
- [ ] 数据库中存在可管理、可停用的真实用户和角色。
- [ ] 所有工单 API 均有身份与数据范围检查。
- [ ] 人工流程可在 AI 完全不可用时独立完成。
- [ ] 工单支持受理、认领、转派、评论、内部备注、解决、确认和重新打开。
- [ ] 状态机拒绝非法转换，并处理并发认领与版本冲突。
- [ ] AI 任务持久化、异步执行、可重试且可恢复。
- [ ] AI 输出保留证据、置信度、模型和 Prompt 版本。
- [ ] 三种角色拥有清楚、独立且可用的前端工作区。
- [ ] PostgreSQL/Redis/Worker/API/Frontend 可通过 Compose 启动。
- [ ] 测试、构建、部署冒烟和真实压测结果有记录。
- [ ] README 与简历不包含未经验证的“企业级”或容量承诺。
